...
GIT_COMMIT=`git show-ref ${BRANCH} | grep heads | cut -d ' ' -f 1`
...
git archive --format=tar ${BRANCH}:config ${CONFIGURATION}.js | tar --directory=${TARGET_DIR}/config -xf -
...
then
	...
	git archive --format=tar ${BRANCH}:config/libs ddl.js | tar --directory=${TARGET_DIR}/config/libs -xf -
...
git archive --format=tar ${BRANCH}:ContinuousIntegration/Deploy/startup-scripts news-service.${CONFIGURATION} | tar --directory=${SYSROOT}/etc/init.d/ -xf -
